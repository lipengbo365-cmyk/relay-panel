...
| 3 | `docker-compose.release.yaml` | the panel image tag — `RELAYPANEL_PANEL_TAG` default (`ghcr.io/lipengbo365-cmyk/relay-panel-panel:x.y.z`) |
...
| 3 | `docker-compose.release.yaml` | the node image tag — `RELAYPANEL_NODE_TAG` default (`ghcr.io/lipengbo365-cmyk/relay-panel-node:x.y.z`) |
...
6. The `v*` tag triggers **docker-release.yml** (PANEL-ONLY): it builds + pushes
   `ghcr.io/lipengbo365-cmyk/relay-panel-panel:X.Y.Z` + `:latest` (multi-arch manifest
...
6. The `node-v*` tag triggers **node-release.yml** (NODE-ONLY): it builds the
   ...
   `ghcr.io/lipengbo365-cmyk/relay-panel-node:X.Y.Z` + `:latest`, creates the node
