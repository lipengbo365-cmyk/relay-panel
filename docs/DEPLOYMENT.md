...
|------|-------------|-------------|
| **Pre-built (default)** | Pulls images from GHCR (`ghcr.io/lipengbo365-cmyk/relay-panel-*:x.y.z`). No compilation on the server. | **Recommended** for production. Fast (~30s). |
...
```bash
git clone https://github.com/lipengbo365-cmyk/relay-panel.git
cd relay-panel
```
