...
impl HealthOrchestrationRepository for PgRepository {
    ...
    async fn find_health_policy(&self, id: i64) -> Result<Option<HealthPolicyRecord>, DbError> {
        Ok(
            sqlx::query_as("SELECT * FROM socks5_check_policies WHERE id=$1")
                .bind(id)
    ...
    async fn update_health_policy(
        ...
        let exists: bool =
            sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM socks5_check_policies WHERE id=$1)")
                .bind(id)
                .fetch_one(&self.pool)
                .await?;
        Err(if exists {
    ...
    async fn cancel_health_job(&self, job_id: &str, now_ms: i64) -> Result<bool, DbError> {
        ...
        sqlx::query("UPDATE socks5_check_job_items SET state='CANCELLED',lease_owner=NULL,lease_expires_at_ms=NULL,finished_at_ms=$1,updated_at_ms=$1 WHERE job_id=$2 AND state IN ('QUEUED','RETRY_WAIT')").bind(now_ms).bind(job_id).execute(&mut *tx).await?;
    ...
    async fn list_released_health_pair_lease_prune_candidates(
        ...
        Ok(sqlx::query_as(
            "SELECT resource_id,relay_node_id FROM socks5_check_pair_leases
             WHERE lease_owner IS NULL AND item_id IS NULL AND updated_at_ms < $1
    ...
    async fn prune_released_health_pair_leases(
        ...
        let result=sqlx::query("DELETE FROM socks5_check_pair_leases WHERE (resource_id,relay_node_id) IN (SELECT resource_id,relay_node_id FROM socks5_check_pair_leases WHERE lease_owner IS NULL AND item_id IS NULL AND updated_at_ms < $1 ORDER BY updated_at_ms LIMIT $2)").bind(cutoff_ms).bind(limit.clamp(0,1000)).execute(&self.pool).await?;
        Ok(result.rows_affected())
    }
