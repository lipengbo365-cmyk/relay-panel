...
if [ "$TRACK" = "panel" ]; then
    ...
    if grep -E "image:.*ghcr\.io/lipengbo365-cmyk/relay-panel-panel.*${VERSION}([^0-9.]|$)" docker-compose.release.yaml >/dev/null 2>&1; then
    ...
    # the node image line.
    if grep -E "image:.*ghcr\.io/lipengbo365-cmyk/relay-panel-node.*${VERSION}([^0-9.]|$)" docker-compose.release.yaml >/dev/null 2>&1; then
